...
from dgraph_ingest.combine import InformationGen
...
def combine_graphs_for_extract(llm: BaseChatModel, graph1: nx.Graph, graph2: nx.Graph, checkpoint_dir: str) -> nx.Graph:
    """
    ...
    rewriter = InformationGen(llm=llm)
    ...
    for node, data in graph2.nodes(data=True):
        if node in combined_graph:
            ...
            combined_data['description'] = rewriter.generate(
                old_information=combined_data.get('description', ''),
                new_information=data.get('description', '')
            )
        else:
