...
def drop_type(client: pydgraph.DgraphClient, type_name: str):
    op = pydgraph.Operation()
    op.drop_op = pydgraph.Operation.DropOp.TYPE
    op.drop_value = type_name
    try:
        client.alter(op)
    finally:
        # Any cleanup code can go here
        print(f"Attempted to drop type: {type_name}")
...
def query_and_ingest_entity(client: pydgraph.DgraphClient, entities: List[Entity]):
    ...
    try:
        ...
        for entity in entities:
            delete_query = f"""
            {{
                getEntity(func: type(Entity)) @filter(eq(id, "{entity.id}")) {{
                    uid
                }}
            }}
            """
            res = txn.query(delete_query)
            existing_entities = json.loads(res.json)

            if len(existing_entities['getEntity']) != 0:
                uid_to_delete = existing_entities['getEntity'][0]['uid']
                txn.mutate(del_obj={'uid': uid_to_delete})
                logger.info(f"Deleted existing entity with UID: {uid_to_delete}")
            ...
            if entity.text_unit_ids:
                for tu in entity.text_unit_ids:
                    query = f"""
                    {{
                        getTextUnit(func: type(TextUnit)) @filter(eq(id, "{tu}")) {{
                            uid
                        }}
                    }}
                    """
...
def query_and_ingest_covariates(client: pydgraph.DgraphClient, covariates: List[Covariate]):
    ...
    try:
        ...
        for cov in covariates:
            delete_query = f"""
            {{
                getCovariate(func: type(Covariate)) @filter(eq(id, "{cov.id}")) {{
                    uid
                }}
            }}
            """
            ...
            if existing_entities['getCovariate']:
                uid_to_delete = existing_entities['getCovariate'][0]['uid']
                txn.mutate(del_obj={'uid': uid_to_delete})
                logger.info(f"Deleted existing entity with UID: {uid_to_delete}")
...
def ingest_community_report(client: pydgraph.DgraphClient, community_reports: List[CommunityReport]):
    ...
    try:
        ...
        for report in community_reports:
            delete_query = f"""
            {{
                getEntity(func: type(CommunityReport)) @filter(eq(community_id, "{report.community_id}")) {{
                    uid
                }}
            }}
            """
...
if __name__ == "__main__":
    ...
    # drop_type(client=client, type_name="CommunityReport")
    # drop_type(client=client, type_name="Covariate")
